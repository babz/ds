...
/**
 * checks energy efficience of engines and manages suspension/activation;
	...
	private void checkEnergyEfficience(
		...
		if (activeEngines.size() > min) {
			if (zeroLoadEngines.size() > 1) {
				...
				activeEngines.get(energyEater).suspendGTE(); // set
																// suspended-flag
			}
		}
		...
		if (activeEngines.size() < max) {
			if ((activeEngines.size() < min)
				...
				// TODO check if engine is really ready (might be no connection
				// after activating)
			}
		}
	}
